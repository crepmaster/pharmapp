/**
 * One-shot cleanup: fully delete legacy test pharmacies and everything
 * attached to them.
 *
 * Usage:
 *   npx ts-node functions/scripts/cleanup-pharmacies.ts <email1> <email2> ...        # dry-run
 *   npx ts-node functions/scripts/cleanup-pharmacies.ts --apply <email1> ...         # execute
 *
 * Requires ADC (gcloud auth application-default login) and project selected.
 *
 * Deletes for each resolved pharmacy uid:
 *   - Auth account
 *   - users/{uid}, pharmacies/{uid}, wallets/{uid}, subscriptions/{uid}
 *   - pharmacy_inventory where pharmacyId == uid
 *   - exchange_proposals where fromPharmacyId == uid OR toPharmacyId == uid
 *   - deliveries where fromPharmacyId == uid OR toPharmacyId == uid
 *   - medicine_requests where requesterId == uid (if collection exists)
 *   - ledger entries where userId == uid
 *   - notifications/{uid} subcollection
 */
...
if (emails.length === 0) {
  console.error("Usage: cleanup-pharmacies.ts [--apply] <email> [<email>...]");
  process.exit(1);
}
...
async function processOne(email: string) {
  console.log(`=== ${email} ===`);
  ...
  // Collection scans
  const scans: Array<{collection: string; where: [string, "==", string][]}> = [
  ...
  for (const scan of scans) {
    let q: FirebaseFirestore.Query = db.collection(scan.collection);
