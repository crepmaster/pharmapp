...
- Références à `pharmacy_app/` et `courier_app/` : ces dossiers ont été **supprimés** (n'existent plus sur disque au 2026-05-12). Seuls `pharmapp_unified/`, `admin_panel/`, `shared/`, `functions/` sont actifs.
- Affirmations sur `devSubscription` / `cleanupTestUser` comme remote orphans : **invalidées** par l'audit `audit-remote-drift.mjs` du 2026-04-22 (0 drift en prod).
- "Bloc 2 — Medicine Requests — CLOS & DÉPLOYÉ" (mars 2026) : le MVP livré est **purchase-only**. L'exchange-mode est explicitement bloqué dans le code et reste à livrer.
- Fonctions `createTrialSubscription` / `migratePharmacySubscriptions` / `checkMigrationStatus` (annoncées 2025-09-18) : **n'existent pas dans le codebase actuel** — implémentation jamais aboutie ou supprimée depuis.
- Comptes test, soldes attendus, ports localhost : peuvent être périmés.

Pour l'état courant du projet, voir [CLAUDE.md](CLAUDE.md).

Cette archive a été créée le 2026-05-12 lors d'un nettoyage de fond (état des lieux clarifié, contradictions résolues).

---

## Snapshot pré-cleanup (contenu original de CLAUDE.md)
