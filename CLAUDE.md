...
- **UI mobile (Sprint 2B.2 à venir)** : registration UI conditionnel sur `MasterDataCountry.licenseRequired`, profile pharmacy license status / correction flow, marketplace listing filter côté Flutter.
...
| **F-LICENSE (2B.2a) Pharmacy UX** | Registration UI conditionnel + LICENSE_REQUIRED handler + profile license status + correction flow | UI mobile inscription mandatory re-prompt `LICENSE_REQUIRED` (contrat 2A.3.1) ; profile license status badge + correction via `submitPharmacyLicense` ; widget tests pharmapp_unified ; **aucun marketplace** | **Prochain sprint** (débloqué par 2B.1, split 2B.2 → 2B.2a + 2B.2b acté 2026-05-13) |
| **F-LICENSE (2B.2b) Marketplace Enforcement** | Listing pharmacies backend-owned + migration 6 consumers Flutter + durcissement `firestore.rules` | Décision `CALLABLE` vs `FLAG` verrouillée dans le contrat (préférence callable sauf triple preuve explorer) ; soit `getMarketplacePharmacies` + `allow list pharmacies` denied client, soit flag `marketplaceVisible` + trigger + scheduler + backfill + rules ; ≥ 7 tests Jest + rules emulator étendu | Bloqué jusqu'à clôture 2B.2a |
