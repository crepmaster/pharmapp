...
- Helper transactionnel idempotent `startTrialForPharmacy` ([functions/src/lib/startTrialForPharmacy.ts](functions/src/lib/startTrialForPharmacy.ts)).
...
- `adminVerifyPharmacyLicense` appelle `startTrialForPharmacy` à `action='verify'`, idempotent (double verify ne re-démarre pas + ne rallonge pas).
- Retrait du `SubscriptionCreationService.createTrialSubscription` côté client pour `UserType.pharmacy` (écrivait dans `subscriptions/{id}` rule-locked backend-only — silencieusement cassé). Courier/admin legacy intact.
...
|---|---|---|---|
| 2026-05-14 | **3 — Trial Subscription aligned with license verification** | Décision architecte verrouillée 2026-05-13 avant run-start : trial 100% backend-owned, retrait du `SubscriptionCreationService.createTrialSubscription` côté client pour `UserType.pharmacy`. Nouveau helper transactionnel idempotent `startTrialForPharmacy` ([functions/src/lib/startTrialForPharmacy.ts](functions/src/lib/startTrialForPharmacy.ts)) + 14 tests Jest. `createPharmacyRegistration` (Sprint 2A.3) initialise désormais les flat fields trial conditionnellement : pays non mandatory → `subscriptionStatus='trial'` + `hasActiveSubscription=true` + dates `now → now+30j` ; pays mandatory + licence fournie → `subscriptionStatus='trial_pending_license'` + `hasActiveSubscription=false` + pas de dates (la pharmacie ne consomme PAS son trial pendant l'attente verify). `adminVerifyPharmacyLicense` (Sprint 2a) appelle `startTrialForPharmacy` uniquement sur `action='verify'`, dans un try/catch qui swallow les erreurs trial pour ne JAMAIS annuler la verify licence (5 tests wire-up). Rules emulator : 3 nouveaux tests `REQ-3-001/002/003` qui prouvent que `'trial_pending_license'` ne matche PAS `hasActiveSubscription()` (le statut bloque create exchange_proposals + create pharmacy_inventory) et que `'trial'` avec endDate futur autorise la création (regression guard). Client Flutter : retrait du call `SubscriptionCreationService.createTrialSubscription` pour `UserType.pharmacy` uniquement (courier/admin legacy intact). UI `subscription_screen.dart` : nouveau banner `trial_pending_license` lu depuis flat fields `pharmacies/{uid}.subscriptionStatus`. Backend total : 247/247 Jest (was 226, +21), rules 34/34 (was 31, +3). Orchestrator run `20260513-214326-e4322f` | (pending commit) |
