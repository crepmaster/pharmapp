...
- **UI mobile pharmacy livré (Sprint 2B.2a, 2026-05-13)** : registration UI conditionnelle sur `MasterDataCountry.licenseRequired` + handler `LICENSE_REQUIRED` (re-prompt licence, snapshot stale OK) ; `PharmacyLicenseStatusSection` (badge 7 statuts + fallback "Pending" unknown) ; `LicenseCorrectionDialog` (correction routée via `submitPharmacyLicense` Sprint 2a). Aucun marketplace touché.
- **Marketplace listing (Sprint 2B.2b à venir)** : nouveau callable `getMarketplacePharmacies` (préférence) OU flag `marketplaceVisible` via trigger + scheduler + backfill + rules durcies ; migration des 6 consumers Flutter (`medicine_requests_screen`, `create_proposal_screen`, `exchange_status_screen`, `subscription_screen`, `inventory_service`, `pharmacy_main_screen`). Hard-block contract : aucun client modifié ne doit pouvoir lister une pharmacie inéligible.
...
| **F-LICENSE (2B.2b) Marketplace Enforcement** | Listing pharmacies backend-owned + migration 6 consumers Flutter + durcissement `firestore.rules` | Décision `CALLABLE` vs `FLAG` verrouillée dans le contrat (préférence callable sauf triple preuve explorer) ; soit `getMarketplacePharmacies` + `allow list pharmacies` denied client, soit flag `marketplaceVisible` + trigger + scheduler + backfill + rules ; ≥ 7 tests Jest + rules emulator étendu | **Prochain sprint** (débloqué par 2B.2a) |
