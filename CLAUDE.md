...
|---|---|---|---|
| 2026-05-14 (+ recette emulator & **real staging** 2026-05-20) | **5 — E2E Closure (✅ PASS)** | Sprint de preuve (pas une feature). 4 livrables : (1) **Truth cleanup** — `docs/testing/NEXT_SESSION_EXCHANGE_TESTING.md` stubbé avec banner Sprint 5 (décrivait encore `createExchangeHold`/`exchangeCapture` comme actifs alors qu'ils sont dead-code). (2) **Plan recette E2E** ([docs/release/SPRINT_5_E2E_CLOSURE_PLAN.md](docs/release/SPRINT_5_E2E_CLOSURE_PLAN.md)) — 8 scénarios obligatoires avec nommage explicite du flow ciblé à chaque test (flow A `createExchangeProposal` vs flow B `medicine_request` purchase/exchange — pas de mélange, lock #3). Couvre license Ghana mandatory (1+2+3), trial gating, marketplace city-scoped, medicine request purchase (4) + exchange Sprint 4 (5, avec preuve runtime lock #5 = 1 inv hold à l'accept), parity matrix cross-mode (6), non-verified blocked sur 5 callables (7), withdrawal happy path + MSISDN validation (8). Critères PASS/CONDITIONAL PASS/BLOCKED documentés. (3) **Script audit Ghana** ([functions/scripts/auditGhanaLicenseReadiness.mjs](functions/scripts/auditGhanaLicenseReadiness.mjs)) — read-only par construction (jamais `set/update/delete`), exige `--project=<id>` explicite (exit 2 sinon), supporte `--out=<csv>`, catégorise pharmacies par `licenseStatus` en 10 buckets (verified_allow, grace_active_allow, missing_status, pending_deny, rejected_deny, correction_needed_deny, expired_deny, grace_expired_deny, not_required_misconfig_deny, unknown_status_deny), redacte `licenseRejectionReason` à 32 chars par défaut, exclut PII (email/phone/licenseNumber) du CSV, émet `AUDIT_SUMMARY_JSON {…}` machine-parsable en dernière ligne. Testé `--help` + `exit 2` sur missing project. (4) **Runbook monitoring 7j** ([docs/release/SPRINT_5_MONITORING_7D.md](docs/release/SPRINT_5_MONITORING_7D.md)) — 7 checks Cloud Logging + Firestore (license gate fail-closed, lock #5 invariant 1 inv hold, courier fee resolved correctly, delivery completion failures, function drift, Ghana admin follow-up, wallet held/deducted divergence), cadence J+0/1/3/7, seuils P0..P3, owner default à définir. Pas de deploy auto (lock #5). **Verdict : CONDITIONAL PASS** — la recette staging n'a pas été exécutée (pas d'environnement staging confirmé au moment du sprint). Le sprint livre tous les artefacts mais le deploy prod reste gated par l'exécution ultérieure de la checklist 8 scénarios. Pré-lock contrat Sprint 5 respecté en intégralité (6 décisions architecte). **Phase 1 emulator recette exécutée 2026-05-20 — 8/8 scénarios PASS (logique)** : S1-S3 via UI Flutter (2026-05-19), S4-S8 via le nouveau driver [functions/scripts/e2eRecette.mjs](functions/scripts/e2eRecette.mjs) qui appelle les vrais callables (functions emulator + tokens Auth réels, pas de raccourci Admin SDK) avec assertions Firestore — run consolidé `e2eRecette.mjs all` = 48/48. Locks prouvés runtime : #1 (0 wallet en barter), #5 (1 seul inv hold à l'accept), #6 (courierFee résolu depuis config), #8 (gate symétrique). 2 bugs prod corrigés via recette : `cancelMedicineRequest` read-after-write tx (`529156a`) + `seedInventory.mjs` medicineIds (`67d89bd`). **Phase 2 real Firebase staging exécutée 2026-05-20 → ✅ PASS** : projet `mediexchange-staging` créé (Blaze), **48 functions déployées** (drift `remote_only=0/local_only=0`, tout nodejs22), `system_config` seedé, recette S1-S8 rejouée via [functions/scripts/e2eRecetteStaging.mjs](functions/scripts/e2eRecetteStaging.mjs) (callables déployés + ID tokens réels `signInWithPassword`) = **44/44 assertions de recette** (à ne pas confondre avec le nombre de functions). Locks #1/#5/#6/#8 prouvés sur infra réelle ; audit Ghana 0 bucket critique. Preuves : [docs/release/evidence/SPRINT_5_staging_2026-05-20/](docs/release/evidence/SPRINT_5_staging_2026-05-20/SUMMARY.md). Hiccups setup résolus : IAM `cloudbuild.builds.builder` au compute SA (build 2nd gen), Identity Platform `initializeAuth` + Email/Password (sinon `auth/configuration-not-found`), `allUsers run.invoker` sur `acceptexchangeproposal` (binding manquant après 1er deploy partiel → 401). Caveat : recette pilotée callables (UI mobile prouvée séparément par widget tests + émulateur phase 1). | (pending commit) |
...
| **5** | Clôture E2E | ✅ **PASS (2026-05-20)** — stratégie hybride architecte exécutée bout-en-bout : phase 1 emulator 8/8 + phase 2 real Firebase staging (`mediexchange-staging`) 8/8 (44/44 assertions). Preuves [docs/release/evidence/SPRINT_5_staging_2026-05-20/](docs/release/evidence/SPRINT_5_staging_2026-05-20/SUMMARY.md). Voir aussi [docs/release/STAGING_SETUP_EMULATOR.md](docs/release/STAGING_SETUP_EMULATOR.md) + [docs/release/STAGING_SETUP_FIREBASE_PROJECT.md](docs/release/STAGING_SETUP_FIREBASE_PROJECT.md) |
