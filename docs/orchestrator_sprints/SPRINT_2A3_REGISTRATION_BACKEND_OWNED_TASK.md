...
- [shared/lib/services/unified_auth_service.dart](../../shared/lib/services/unified_auth_service.dart) `signUp` : nouvelle branche `if (userType == UserType.pharmacy)` qui appelle le callable via `FirebaseFunctions.instanceFor(region: 'europe-west1')`, puis `signInWithEmailAndPassword` pour obtenir la session. Sur erreur, la pharmacy branch fait **`rethrow`** de la `FirebaseFunctionsException` originale (cf. correctif 2A.3.1) — le `details.code === 'LICENSE_REQUIRED'` propage intact jusqu'à l'UI Sprint 2B, ce qui permet le re-prompt licence immédiat. Courier/admin gardent le flow client-write existant.
