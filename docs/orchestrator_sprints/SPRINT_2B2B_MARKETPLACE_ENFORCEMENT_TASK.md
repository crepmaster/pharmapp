...
✅ **Livré 2026-05-13** (orchestrator run `20260513-205611-69104d`). F-LICENSE end-to-end **clos**.

### Décision architecte verrouillée — CALLABLE

L'explorer 2B.2b a retenu l'option CALLABLE (titre du coder prompt : *"Backend-Owned Marketplace Listing (CALLABLE Path)"*) parce que :

- La triple preuve qui aurait justifié FLAG n'est pas réunie : un seul consumer Flutter sur 6 fait réellement du marketplace listing (`inventory_service.getAvailableMedicines`), donc le bénéfice "consumers simples → gain perf flag-trigger" ne pèse pas le coût trigger + scheduler + backfill + rules.
- Callable est *frais par construction* : il évalue `licenseRequired` flip, unknown country, grace expired à chaque appel — pas d'état matérialisé à maintenir, pas de scheduled function pour les expirations grace_period.

### Backend (Lot 1)

- **Nouveau callable `getMarketplacePharmacies`** ([functions/src/getMarketplacePharmacies.ts](../../functions/src/getMarketplacePharmacies.ts)) :
  - `onCall`, region `europe-west1`, `context.auth` required, sinon `HttpsError("unauthenticated")`.
  - Input `{ countryCode: string, cityCode?: string }` validé via helper pur `validateGetMarketplacePharmaciesInput` (countryCode `/^[A-Z]{2}$/`, cityCode non-vide si présent).
  - Charge `system_config/main.countries[countryCode]`. Fail-closed sur unknown country / system_config absent → zéro résultat + `logger.warn` structuré avec `callerUid` et `countryCode`.
  - Query `pharmacies` filtré par `countryCode` (+ `cityCode` si fourni). Pour chaque doc, appelle `evaluateLicenseGate` (Sprint 2A.3) avec `resolution: { status: 'loaded', country }` ; ne retient que `decision === 'allow'`.
  - Helper pur `projectListingSafe` qui restreint l'output à `{uid, pharmacyName, address, countryCode, cityCode?, city?, phoneNumber?, locationData?}`. Strictement aucun champ licence ni reason de gate ne sort vers le client.
- **Export** ajouté dans [functions/src/index.ts](../../functions/src/index.ts) sous nouvelle section "Marketplace Listing (Sprint 2B.2b)".
- **Tests Jest** : [functions/src/\_\_tests\_\_/getMarketplacePharmacies.test.ts](../../functions/src/__tests__/getMarketplacePharmacies.test.ts) — **18 tests** (largement au-dessus du minimum architecte de 7) :
  - 4 tests `validateGetMarketplacePharmaciesInput` pur (countryCode required / shape / valid+optional cityCode / empty cityCode).
  - 2 tests `projectListingSafe` pur (strips license fields × absence des optionnels).
  - **7 architect-locked matrix** : verified visible, pending_verification hidden, grace_period actif visible, grace_period expiré hidden, rejected hidden, country non mandatory toutes visibles, unknown country zéro + warn.
  - 5 edge cases : output safety (licenseStatus/Reason/Number/Url strippés), unauthenticated → throw, invalid countryCode → throw, system_config/main absent → fail-closed + warn, cityCode filter narrows.
- **Suite backend totale** : **222/222 Jest** (was 204 → +18), build + lint clean.

### Firestore rules (Lot 2)

- [firestore.rules](../../firestore.rules) `match /pharmacies/{userId}` : `allow read: if isAuthenticated()` remplacé par **`allow get: if isAuthenticated()` + `allow list: if false`**. Commentaire inline référence Sprint 2B.2b. Admin SDK bypasse les deux (callables backend conservent l'accès via service account).
- **Rules emulator tests** : [functions/src/\_\_tests\_\_/firestore-rules.test.ts](../../functions/src/__tests__/firestore-rules.test.ts) étendu avec 5 nouveaux tests :
  - **REQ-2B2B-001** : `getDocs(collection('pharmacies'))` authenticated → DENIED.
  - **REQ-2B2B-002** : `getDocs(query(collection, where(countryCode, ==, 'GH')))` → DENIED.
  - **REQ-2B2B-003** : `getDoc(doc('pharmacies/uid'))` → ALLOWED (lookup pour profile / correction / cross-pharmacy).
  - **REQ-2B2B-004** : unauthenticated client list → DENIED.
  - **REQ-2B2B-005** : admin SDK list via `withSecurityRulesDisabled` → ALLOWED (bypass confirmé pour callable backend).
- **Suite rules** : **27/27** verts (was 22 → +5).

### Flutter migration (Lot 3) — audit listing vs lookup

Audit explicite des 6 consumers listés au contrat (`grep -rn "collection('pharmacies')"` documenté inline dans [pharmapp_unified/test/services/inventory_service_marketplace_listing_test.dart](../../pharmapp_unified/test/services/inventory_service_marketplace_listing_test.dart)) :

| Fichier | Type | Action |
|---|---|---|
| `inventory_service.dart` | LISTING (`.where(cityCode \| city, ...)`) | **Migré** via seam callable |
| `medicine_requests_screen.dart` | LOOKUP `.doc(uid).get()` | Inchangé (`allow get` couvre) |
| `create_proposal_screen.dart` | LOOKUP `.doc(user.uid).get()` | Inchangé |
| `exchange_status_screen.dart` | LOOKUP `.doc(pharmacyId).get()` | Inchangé |
| `subscription_screen.dart` | LOOKUP `.doc(user.uid).get()` | Inchangé |
| `pharmacy_main_screen.dart` | LOOKUP `.doc(uid).get()` | Inchangé |

Sur 6 consumers, **un seul** faisait du marketplace listing. Les 5 autres restent sur `allow get` Firestore direct — les rules emulator REQ-2B2B-003 prouvent que `getDoc` par UID continue de fonctionner.

- [pharmapp_unified/lib/services/inventory_service.dart](../../pharmapp_unified/lib/services/inventory_service.dart) — Sprint 2B.2b :
  - Nouveau typedef `MarketplacePharmaciesFetcher` exporté.
  - Nouveau champ statique `@visibleForTesting fetchMarketplacePharmacyIds` (production = `_fetchMarketplacePharmacyIdsFromCallable`) qui appelle `FirebaseFunctions.instanceFor(region: 'europe-west1').httpsCallable('getMarketplacePharmacies').call(...)` et déballe la liste d'UIDs.
  - `getAvailableMedicines` ne fait plus de `.where(cityCode | city, ...)` direct sur `collection('pharmacies')`. À la place, lit son propre `countryCode`, appelle `fetchMarketplacePharmacyIds(countryCode, cityCode)`, filtre `self`, chunke les UIDs et conserve la query existante sur `pharmacy_inventory.where(availableForExchange + pharmacyId in chunk)`.
  - Try/catch autour de l'appel callable : si throw → log + yield empty (UX non-cassante).

### Tests Flutter (Lot 4)

- [pharmapp_unified/test/services/inventory_service_marketplace_listing_test.dart](../../pharmapp_unified/test/services/inventory_service_marketplace_listing_test.dart) (NEW) :
  - 4 tests `InventoryService.fetchMarketplacePharmacyIds` : forwarding `countryCode` + `cityCode` au fetcher, `null` cityCode optionnel, fetcher empty list honoré, seam reversibility via tearDown.
  - Audit manifest inline en commentaire de fin de fichier listant les 6 consumers + leur classification listing vs lookup, en regard du diff.

### Documentation (Lot 5)

- `CLAUDE.md` : Sprint 2B.2b ajouté au tableau historique sprints ; backlog F-LICENSE 2B.2b marqué LIVRÉ + mention "**F-LICENSE end-to-end CLOS**" ; section "ce qui n'est PAS livré" §2 réécrite : registration + profile + marketplace tous livrés. La feature licence est désormais fonctionnellement complète.
- `docs/ACTIVE_DOCS.md` : 2B.2b → "Sprints closed". Sprint 3 (Trial Subscription) débloqué.
- `docs/orchestrator_sprints/README.md` : 4.2.b fermé.
- Ce contrat : section "Statut final" présente.

### Validations finales

- `cd functions && npm run build && npm run lint && npm test` : **222/222 Jest** (was 204), build OK, lint clean.
- `cd functions && npm run test:rules` : **27/27** rules emulator (was 22).
- `cd shared && dart analyze` : pas de changement.
- `cd admin_panel && flutter analyze && flutter test` : pas de changement (Sprint 2B.1 inchangé).
- `cd pharmapp_unified && flutter analyze` : clean sur les fichiers touchés.
- `cd pharmapp_unified && flutter test test/services/inventory_service_marketplace_listing_test.dart` : 4/4 verts.
- `git diff --check` : clean.

### Non-régressions

- Sprint 2a / 2A.1 / 2A.2 / 2A.3 callables licence : inchangés. Le gate `evaluateLicenseGate` est consommé tel quel.
- Sprint 2B.1 admin operations : aucun changement (admin_panel intact).
- Sprint 2B.2a pharmacy UX : aucun changement.
- 5 lookups par UID (medicine_requests, create_proposal, exchange_status, subscription, pharmacy_main, profile) : préservés par `allow get` Firestore direct.
- Pas d'upload Firebase Storage. Pas de migration de schema. Pas de touche `admin_panel/**` ni `shared/lib/**`.
- Aucun deploy prod. Aucune mutation prod.

### Pré-deploy (rappel `TD-LICENSE-REGISTRATION-AUDIT`)

Lancer `functions/scripts/auditUnknownCountryPharmacies.mjs` avant push prod du callable — toute pharmacie avec `countryCode` absent de `system_config/main.countries` sera maintenant aussi exclue du marketplace listing par fail-closed. Décision produit nécessaire si comptes affectés en prod.
